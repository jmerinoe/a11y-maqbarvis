...
The instructions screen SHALL state the task ("Realiza un flujo de compra de una camiseta azul, talla M, utilizando la tarjeta 4000056655665556"), that the timer starts on Continuar, stops automatically on completing that purchase, and that a congrats message and top-10 ranking follow. The timer SHALL NOT start on screen render or experience selection — only on the Continuar button, which SHALL record `startedAt` and navigate to `#/home`.
